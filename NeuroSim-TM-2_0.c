...
#define PARAMETER_N 73   //number of parameters expected to be in the input file
...
void setCoordinates(struct neuron **neurons, int layer_type, int N) {
	...
	int *tmp_list = (int*)malloc(MAXIMUM_NUMBER_OF_NEURONS * sizeof(int));
...
int main(int argc, char *argv[]) {
	...
	struct neuron **neurons = (neuron**)malloc(MAXIMUM_NUMBER_OF_NEURONS * sizeof(struct neuron*));
	int i, j, CONN_PER_NEURON_COUNT, layer_type, COORDINATES_LOAD_TYPE = 0, SYNAPTIC_DATA_LOAD_TYPE = 0, STIMULATION_DATA_LOAD_TYPE = 0, W_OUTPUT_COUNTER = 1,
		TOPOLOGY_LOAD_TYPE = 0, N_SP, N_ACTIVE_CONN, N_con = 0, BG_CURRENT_NOISE_MODE = 0;
	int burst_flag, burst_counter, eof_flag, eof_flag2, INH_COUNT, EXC_COUNT, N_COUNT, FORCE_BINOMIAL_TOPOLOGY = 0;
	...
	double avg_x, avg_y, avg_z, avg_u;
	...
	FILE *input = fopen("C:\\Users\\Админ\\Downloads\\input.txt", "r");
	...
	printf("********************NeuroSimTM-2.0*****************\n");
	printf("version 12.07.2016\n\n");
	...
	for (i = 0; i<N; i++) {
		neurons[i] = (neuron*)malloc(sizeof(struct neuron));
		neurons[i]->V = V_REST;
		...
		neurons[i]->out_conn = (synapse*)malloc(sizeof(struct synapse));
		neurons[i]->out_conn->next = NULL;
		neurons[i]->out_conn->post_id = -1;

		neurons[i]->in_conn = (inc_synapse*)malloc(sizeof(struct inc_synapse));
	...
	for (i = 0; i<N; i++) {
		...
		for (j = 0; j<N; j++) {
			...
			if (j != i && ((TOPOLOGY_LOAD_TYPE == READ_TOPOLOGY_FROM_FILE && i == abs(pre_num) && j == abs(post_num)) || (TOPOLOGY_LOAD_TYPE == GENERATE_TOPOLOGY))) {
				...
				if ((layer_type != LAYER_TYPE_BINOMIAL && FORCE_BINOMIAL_TOPOLOGY != 1 && prob < my_theta(max_conn_length - sqrt(pow(neurons[i]->x - neurons[j]->x, 2) + pow(neurons[i]->y - neurons[j]->y, 2)))*exp(-sqrt(pow(neurons[i]->x - neurons[j]->x, 2) + pow(neurons[i]->y - neurons[j]->y, 2)) / lambda)) ||
					...
					if (STDP_status) tmp_synapse->w = w_initial;
					...
					tmp_input_synapse->next = (inc_synapse*)malloc(sizeof(struct inc_synapse));
					tmp_input_synapse = tmp_input_synapse->next;
					tmp_input_synapse->syn_pointer = NULL;
					tmp_input_synapse->next = NULL;

					// moving pointer to the next list item
					tmp_synapse->next = (synapse*)malloc(sizeof(struct synapse));
		...
		if (i % 100 == 0)
			printf("Connections done for %d neurons...\n", i);
	}
 ...
 clock_t begin = clock();
	...
	for (simulation_time = 0; simulation_time < SIM_TIME; simulation_time += dt) {
		...
		for (i = 0; i<N; i++) {
			...
			// так как две предыдущие команды делаются для всех нейронов, стоит вынести до цикла и сделать параллельно
			...
			//синапсы и таймеры - это уже имеющиеся списки. таймер - просто массив, а вот список синапсов хранит номер, который нужно будет использовать для обращения по адресу

			tmp_input_synapse = neurons[i]->in_conn; // нужно поэкспериментировать, чтобы понять - ссылка или копия
			while (tmp_input_synapse->syn_pointer != NULL) { // банальный проход по всему циклу
				stepSynapse(tmp_input_synapse->syn_pointer, dt); // инлайн? в любом случае, кандидат на параллелизацию

				if (tmp_input_synapse->syn_pointer->timers != NULL) {

					tmp_timer = tmp_input_synapse->syn_pointer->timers;

					if (tmp_timer->next != NULL) {
		...
		for (i = 0; i<N; i++) {
			...
			if (neurons[i]->V >= V_TH) { //In case of exceeding threshold value neuron emits a spike
				...
				printf("Neuron %d emitted spike at %g ms \t|| Bursts registered: %d\n", i, simulation_time, burst_counter);
				...
				while (tmp_synapse->post_id != -1) {
					...
					tmp_synapse->timers = (delay_timer*)malloc(sizeof(struct delay_timer));
				...
				if (neurons[i]->type == TYPE_EXCITATORY) neurons[i]->ref_time_left = 3;
				else neurons[i]->ref_time_left = 2;
		...
		if (STDP_status != STDP_IS_OFF) {
			...
			if (w_output_timer <= 0) {
				...
				if (layer_type == LAYER_TYPE_BARBELL) { //writing weights for "right" and "left" connections separately in case of BARBELL topology
					...
					////strcat(tmp_str, _itoa(W_OUTPUT_COUNTER, tmp_str_2, 10));
					////strcat(tmp_str, ".txt");
		...
		if (stim_disable_protocol != 0 && burst_detection_timer <= 0) {
			if (stim_disable_protocol == 1 && burst_counter == pb_number - 1 && activity >= activity_threshold) {
				for (i = 0; i<N; i++) {
					neurons[i]->I_b = 0;
					neurons[i]->p_sp = 0;
				}
